...
public class SettingsProvider extends ContentProvider {
    ...
    private static final SparseArray<SettingsCache> sSecureCaches
            = new SparseArray<SettingsCache>();
   private static final SparseArray<SettingsCache> sREVOLTCaches
            = new SparseArray<SettingsCache>();
        ...
        SqlArguments(Uri url, String where, String[] args) {
            if (url.getPathSegments().size() == 1) {
                ...
                if (TABLE_SYSTEM.equals(this.table) || TABLE_SECURE.equals(this.table) ||
                    TABLE_GLOBAL.equals(this.table) || TABLE_REVOLT.equals(this.table)) {
    ...
    private void sendNotify(Uri uri, int userHandle) {
        ...
        if (table.equals(TABLE_SYSTEM)) {
            ...
            backedUpDataChanged = true;
        } else if (table.equals(TABLE_REVOLT)) {
            property = Settings.REVOLT.SYS_PROP_SETTING_VERSION;
            backedUpDataChanged = true;
        } else if (isGlobal) {
    ...
    void onUserRemoved(int userHandle) {
        synchronized (this) {
            ...
            sREVOLTCaches.delete(userHandle);
    ...
    private void establishDbTracking(int userHandle) {
        ...
        synchronized (this) {
            ...
            if (dbhelper == null) {
                ...
                sREVOLTCaches.append(userHandle, new SettingsCache(TABLE_REVOLT));
    ...
    private void fullyPopulateCaches(final int userHandle) {
        ...
        fullyPopulateCache(dbHelper, TABLE_REVOLT, sREVOLTCaches.get(userHandle));
    }
    ...
    public SettingsCache cacheForTable(final int callingUser, String tableName) {
        ...
        if (TABLE_REVOLT.equals(tableName)) {
            return getOrConstructCache(callingUser, sREVOLTCaches);
        }
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        if (Settings.CALL_METHOD_GET_REVOLT.equals(method)) {
            if (LOCAL_LOGV) Slog.v(TAG, "call(revolt:" + request + ") for " + callingUser);
            dbHelper = getOrEstablishDatabase(callingUser);
            cache = sREVOLTCaches.get(callingUser);
            return lookupValue(dbHelper, TABLE_REVOLT, cache, request);
        }
        ...
        } else if (Settings.CALL_METHOD_PUT_REVOLT.equals(method)) {
            if (LOCAL_LOGV) Slog.v(TAG, "call_put(revolt:" + request + "=" + newValue + ") for " + callingUser);
            insertForUser(Settings.REVOLT.CONTENT_URI, values, callingUser);
        } else {
