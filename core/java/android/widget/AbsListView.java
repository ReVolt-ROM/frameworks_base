    ...
    void reportScrollStateChange(int newState) {
        if (newState != mLastScrollState) {
            ...
            if (newState == OnScrollListener.SCROLL_STATE_IDLE) {
                ...
                mListAnimationMode = Settings.REVOLT.getIntForUser(
                        mContext.getContentResolver(),
                        Settings.REVOLT.LISTVIEW_ANIMATION,
                        0, UserHandle.USER_CURRENT_OR_SELF);
                if (mListAnimationMode != 0) {
                    mListAnimationInterpolatorMode = Settings.REVOLT.getIntForUser(
                            mContext.getContentResolver(),
                            Settings.REVOLT.LISTVIEW_INTERPOLATOR,
                            0, UserHandle.USER_CURRENT_OR_SELF);
                }
            }
            if (mOnScrollListener != null) {
