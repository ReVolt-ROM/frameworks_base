...
public class RibbonGestureCatcherView extends LinearLayout {
    ...
    protected void updateSettings() {
        ...
        mDragButtonColor = Settings.REVOLT.getInt(cr, SETTINGS_REVOLT[AokpRibbonHelper.HANDLE_COLOR], Color.TRANSPARENT);
        mButtonWeight = Settings.REVOLT.getInt(cr, SETTINGS_REVOLT[AokpRibbonHelper.HANDLE_WEIGHT], 30);
        mButtonHeight = Settings.REVOLT.getInt(cr, SETTINGS_REVOLT[AokpRibbonHelper.HANDLE_HEIGHT], 50);
        mLongSwipeAction = Settings.REVOLT.getString(cr, SETTINGS_REVOLT[AokpRibbonHelper.LONG_SWIPE]);
        if (TextUtils.isEmpty(mLongSwipeAction)) {
            mLongSwipeAction = AwesomeConstant.ACTION_APP_WINDOW.value();
            Settings.REVOLT
                    .putString(cr, SETTINGS_REVOLT[AokpRibbonHelper.LONG_SWIPE], AwesomeConstant.ACTION_APP_WINDOW.value());
        }
        mLongPressAction = Settings.REVOLT.getString(cr, SETTINGS_REVOLT[AokpRibbonHelper.LONG_PRESS]);
        if (TextUtils.isEmpty(mLongPressAction)) {
            mLongPressAction = AwesomeConstant.ACTION_APP_WINDOW.value();
            Settings.REVOLT
                    .putString(cr, SETTINGS_REVOLT[AokpRibbonHelper.LONG_PRESS], AwesomeConstant.ACTION_APP_WINDOW.value());
        }
        vib = Settings.REVOLT.getBoolean(cr, SETTINGS_REVOLT[AokpRibbonHelper.HANDLE_VIBRATE], true);
        mLocation = Settings.REVOLT.getInt(cr, SETTINGS_REVOLT[AokpRibbonHelper.HANDLE_LOCATION], 0);
        updateLayout();
    }
}
