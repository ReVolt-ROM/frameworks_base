...
public class AppTransition implements Dump {
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(
                    Settings.REVOLT.getUriFor(Settings.REVOLT.ANIMATION_CONTROLS_DURATION), false, this);
            resolver.registerContentObserver(
                    Settings.REVOLT.getUriFor(Settings.REVOLT.ANIMATION_CONTROLS_NO_OVERRIDE), false, this);
            resolver.registerContentObserver(
                    Settings.REVOLT.getUriFor(Settings.REVOLT.ANIMATION_CONTROLS_EXIT_ONLY), false, this);
            resolver.registerContentObserver(
                    Settings.REVOLT.getUriFor(Settings.REVOLT.ANIMATION_CONTROLS_REVERSE_EXIT), false, this);
            for (int i = 0; i < 10; i++) {
	            resolver.registerContentObserver(
                    Settings.REVOLT.getUriFor(Settings.REVOLT.ACTIVITY_ANIMATION_CONTROLS[i]), false, this);
            }
        }
         @Override
    ...
    private void updateSettings() {
        ContentResolver resolver = mContext.getContentResolver();
        for (int i = 0; i < 10; i++) {
            mActivityAnimations[i] = Settings.REVOLT.getInt(resolver, Settings.REVOLT.ACTIVITY_ANIMATION_CONTROLS[i], 0);
        }
        ...
        int temp = Settings.REVOLT.getInt(resolver, Settings.REVOLT.ANIMATION_CONTROLS_DURATION, 0);
